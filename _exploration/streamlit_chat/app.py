...
st.header("Integrate Generative AI with Enterprise Knowledge")
...
uploaded_files = st.file_uploader(
    """
    Upload your files! You may include PDFs, txt, docx and HTML files 😎
...
if len(DOCUMENTS_TO_CHOOSE_FROM) == 0:
    ...
    if QUERY:
        ...
        messages =[
                    ...
                    #{"role": "user", "content": history_message.format(history = '\n'.join(history))},
                    {"role": "user", "content": question_message.format(question=QUERY)}
                    ]
        
        response = generate_response(messages, MODEL, TEMPERATURE, MAX_TOKENS)

        st.write(f"{response}")

        st.write('\n')
        st.write("Here are the sources that have been identified as relevant:")
        st.text(sources)
